...
exports.ReceivedAmountContract = function(req, res) {
  ...
  .then(objCus => {
      ...
      // tinh tien phat ngay va luon cho cac phat sinh tien phat sau ngay 08/12/2018
      let ContractDate = parseInt(moment(objCus.ContractDate,'DD-MM-YYYY').format('YYYYMMDD'));
      if (ContractDate > 20181208){
        if (objData.Amount > objCus.PenaltyAmount){
          objData.Amount -= objCus.PenaltyAmount;
          objCus['RealPenaltyAmount'] = objCus.PenaltyAmount;
          objCus.PenaltyAmount = 0;
        }else{
          objCus['RealPenaltyAmount'] = objData.Amount;
          objData.Amount =0;
          objCus.PenaltyAmount -= objCus['RealPenaltyAmount'];
        }
        objCus['RealPenaltyDate'] = objData.PaymentDate;
      }
...
exports.updateOverDueDate = function(req, res) {
  ...
  .then(contracts => {
    ...
    for (let i=0;i<contracts.length;i++){
      ...
      let objContract = contracts[i];
      if (objContract.ContractId !== 'B0419RLFX') continue;
      let currentDate = moment(Date.now()).format('YYYYMMDD');
      currentDate = moment(currentDate,'YYYYMMDD');
      let currentPeriodCount = parseInt(moment(Date.now()).format('YYYYMM'));
      let isFlag = false;
